...
export class Update implements OnInit {
  ...
  loadData(): void {
    this.loading = true;
    this.HotelService.getById(this.entityId).subscribe({
      next: (data: HotelResponseI) => {
  ...
  submit(): void {
    if (this.form.valid) {
      ...
      this.HotelService.update(this.entityId, formData).subscribe({
        next: (response: HotelResponseI) => {
          this.messageService.add({ severity: 'success', summary: 'Éxito', detail: 'Hotel actualizado correctamente' });
          setTimeout(() => this.router.navigate(['/Hotel']), 1000);
        },
        error: (error: any) => {
