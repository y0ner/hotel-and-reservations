...
function add_crud_permissions_to_admin() {
    ...
    # 2. Detectar automáticamente nombres de recursos desde backend routes
    echo -e "\n${CYAN}2. Detectando recursos automáticamente desde 'backend-Hotel/src/routes'...${NC}"
    ...
    detect_resource_names() {
        ...
        local route_files=$(find "$routes_dir" -type f -name "*.ts" ! -name "index.ts")

        for f in $route_files; do
            # Grep for routes, extract the path after /api/, remove params, and get unique names
            local paths=$(grep -oP 'app\.route\("/api/\K[^"]+' "$f" | sed 's/\/:.*//' | sort -u)
            
            for path in $paths; do
                # Lowercase the name and add to the list
                local lname=$(echo "$path" | awk '{print tolower($0)}')
    ...
    if [[ -z "$resource_names" ]]; then
        # use detection
        mapfile -t detected < <(detect_resource_names)
        if [[ ${#detected[@]} -eq 0 ]]; then
            echo -e "${RED}No se detectaron rutas en '$routes_dir'. Cancelling.${NC}"
            return
        fi
        resource_names="${detected[*]}"
        echo -e "${GREEN}Detectado: ${resource_names}${NC}"
    fi

    # 3. Iterar sobre cada name y crear los 5 endpoints CRUD
...
ensure_initial_entities() {
    echo -e "\n${CYAN}Verificando existencia de usuario y rol iniciales...${NC}"

    # Verificar usuarios
    local users_resp=$(make_request "GET" "/users")
    if ! echo "$users_resp" | jq -e '.users | length > 0' > /dev/null 2>&1; then
        echo -e "${YELLOW}No se encontraron usuarios. Creando usuario por defecto 'admin'...${NC}"
        read -p "Email para usuario admin (por defecto admin@gmail.com): " default_email
        default_email=${default_email:-admin@gmail.com}
        read -p "Nombre de usuario (por defecto admin): " default_username
        default_username=${default_username:-admin}
        read -s -p "Contraseña (por defecto admin123): " default_password
        echo
        default_password=${default_password:-admin123}

        local user_data
        user_data=$(printf '{"username": "%s", "email": "%s", "password": "%s", "is_active": "ACTIVE", "avatar": "avatar.jpg"}' "$default_username" "$default_email" "$default_password")
        local create_response=$(make_request "POST" "/register" "$user_data")
        echo "$create_response" | jq .
        # Try to extract user id from response (flexible keys)
        local new_user_id=$(echo "$create_response" | jq -r '.user.id // .user_interface.id // .id')
        if [[ -n "$new_user_id" && "$new_user_id" != "null" ]]; then
            echo -e "${GREEN}Usuario creado con ID: $new_user_id. Credenciales: ${default_email} / ${default_password}${NC}"
            # Attempt to assign Administrator role to the created user
            local roles_public=$(make_request "GET" "/roles/public")
            local admin_role_id=$(echo "$roles_public" | jq -r '.roles[] | select(.name=="Administrator") | .id')
            if [[ -n "$admin_role_id" && "$admin_role_id" != "null" ]]; then
                echo -e "${YELLOW}Asignando rol 'Administrator' al usuario creado...${NC}"
                local assign_data=$(printf '{"role_id": %s, "user_id": %s, "is_active": "ACTIVE"}' "$admin_role_id" "$new_user_id")
                local assign_resp=$(make_request "POST" "/roleUsers/public" "$assign_data")
                echo "$assign_resp" | jq .
                echo -e "${GREEN}Asignación intentada.${NC}"
            else
                echo -e "${RED}No se encontró el rol Administrator para asignar.${NC}"
            fi
        else
            echo -e "${RED}No se pudo extraer el ID del usuario creado. Revisa la respuesta anterior.${NC}"
        fi
    else
        echo -e "${GREEN}Usuarios detectados: no es necesario crear usuario por defecto.${NC}"
    fi

    # Verificar roles (endpoint público)
    local roles_resp=$(make_request "GET" "/roles/public")
    if ! echo "$roles_resp" | jq -e '.roles | length > 0' > /dev/null 2>&1; then
        echo -e "${YELLOW}No se encontraron roles. Creando rol 'Administrator'...${NC}"
        local role_data='{"name": "Administrator", "is_active": "ACTIVE"}'
        local role_response=$(make_request "POST" "/roles/public" "$role_data")
        echo "$role_response" | jq .
    else
        # comprobar si Administrator existe
        local admin_id=$(echo "$roles_resp" | jq -r '.roles[] | select(.name=="Administrator") | .id')
        if [[ -n "$admin_id" && "$admin_id" != "null" ]]; then
            echo -e "${GREEN}Rol 'Administrator' ya existe (ID: $admin_id).${NC}"
        else
            echo -e "${YELLOW}Rol 'Administrator' no encontrado. Creándolo...${NC}"
            local role_data='{"name": "Administrator", "is_active": "ACTIVE"}'
            local role_response=$(make_request "POST" "/roles/public" "$role_data")
            echo "$role_response" | jq .
        fi
